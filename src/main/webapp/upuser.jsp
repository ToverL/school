...
<h1 align="center" style="color: red;font-size: large">此权限为管理员专有，只可上传导入标准格式的Excl表格，导入前请严格审核！</h1>
<table class="filetable">
    ...
    <c:forEach items="${disk}" var="list">
        <tr class="tr_click">
            ...
            <td><a href="UpUserExcel.do?diskid=${list.diskid}" onclick="if(confirm('确认上传吗？(请谨慎操作)')==false)return false;">上传</a>&nbsp;<a href="delete.do?diskid=${list.diskid}&diskuserid=${list.diskuserid}" onclick="if(confirm('确认删除吗？(请谨慎操作)')==false)return false;">删除</a></td>
            <td></td>
        </tr>
    </c:forEach>
